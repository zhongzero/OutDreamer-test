...
from outdreamer.dataset.transform import ToTensorVideo
from outdreamer.models.causalvideovae import ae_norm
...
class OutpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        self,
        given_video: torch.FloatTensor, # (T C H W)
        preGen_video: Optional[torch.FloatTensor] = None, # (T2 C H W)
        mask_ratio_l: float = 0.3,
        mask_ratio_r: float = 0.3,
        mask_ratio_u: float = 0.0,
        mask_ratio_d: float = 0.0,
        conditional_scale: float = 1.0,
        ...
        **kwargs,
        ...
        if mask_ratio_l != 0.0 or mask_ratio_r != 0.0:
            start_h, start_w = 0, math.floor(W * mask_ratio_l)
            end_w = math.floor(W * mask_ratio_r)
            mask_h_size = H
            mask_w_size = W - start_w - end_w
            print(f"mask_ratio_l={mask_ratio_l}, mask_ratio_r={mask_ratio_r}")
        else :
            start_h, start_w = math.floor(H * mask_ratio_u), 0
            end_h = math.floor(H * mask_ratio_d)
            mask_h_size = H - start_h - end_h
            mask_w_size = W
        ...
        input_mask = torch.zeros_like(masked_video)
        # input_mask: (B,C,T,H,W)
        input_mask[:, :, -T1:, start_h:start_h+mask_h_size, start_w:start_w+mask_w_size] = 1
        if preGen_video != None:
            T2 = preGen_video.shape[0]
            print(f"T2={T2}")
            print("preGen_video.shape: ", preGen_video.shape)
            input_mask[:, :, :T2] = 1
        
        masked_video = masked_video * input_mask
        
        masked_video = self.vae.encode(masked_video.to(self.vae.vae.dtype)).to(latents.dtype) # B C T H(h) W(w)
        input_mask = self.vae.encode(input_mask.to(self.vae.vae.dtype)).to(latents.dtype) # B C T H(h) W(w)
        ...
        control_cond = torch.cat([masked_video, input_mask], dim=1)
        control_cond = torch.cat([control_cond] * 2) if do_classifier_free_guidance else control_cond
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.transformer(
                    ...
                    control_scale=conditional_scale,
    ...
    def decode_latents(self, latents):
        if self.vae.vae.dtype == torch.bfloat16:
            self.vae = self.vae.to(torch.float16) # Ascend is not support for bfloat16
