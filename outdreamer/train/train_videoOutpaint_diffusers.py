...
from outdreamer.adaptor.modules import replace_with_fp32_forwards

try:
    import torch_npu
    from outdreamer.npu_config import npu_config
    from outdreamer.acceleration.parallel_states import initialize_sequence_parallel_state, \
        destroy_sequence_parallel_group, get_sequence_parallel_state, set_sequence_parallel_state
    from outdreamer.acceleration.communications import prepare_parallel_data, broadcast
except:
    torch_npu = None
    npu_config = None
    from outdreamer.utils.parallel_states import initialize_sequence_parallel_state, \
        destroy_sequence_parallel_group, get_sequence_parallel_state, set_sequence_parallel_state
    from outdreamer.utils.communications import prepare_parallel_data, broadcast
    pass
import time
...
from outdreamer.dataset import getdataset
from outdreamer.models import CausalVAEModelWrapper
from outdreamer.models.text_encoder import get_text_enc, get_text_warpper
from outdreamer.models.causalvideovae import ae_stride_config, ae_channel_config
from outdreamer.models.causalvideovae import ae_norm, ae_denorm
from outdreamer.models.diffusion import Diffusion_models, Diffusion_models_class
from outdreamer.utils.dataset_utils import Collate, LengthGroupedSampler
...
def main(args):
    ...
    model.control_conv.requires_grad_(True)
    ...
    def run(model_input, model_kwargs, prof):
        ...
        C = model_input.shape[1] # 12
        C0 = C // 3 # 12/3=4

        noise = torch.randn_like(model_input[:, 0:C0])
        if args.noise_offset:
            # https://www.crosslabs.org//blog/diffusion-with-offset-noise
            noise += args.noise_offset * torch.randn((model_input.shape[0], C0, 1, 1, 1),
        ...
        if noise_scheduler.config.prediction_type == "epsilon":
            ...
            target = noise_scheduler.get_velocity(model_input[:, 0:C0], noise, timesteps)
        elif noise_scheduler.config.prediction_type == "sample":
            # We set the target to latents here, but the model_pred will return the noise sample prediction.
            target = model_input[:, 0:C0]
        ...
        if timesteps <= 200 and noise_scheduler.config.prediction_type == "epsilon":
            ...
            mean1 = x_0_hat.mean(dim=(3,4))
            var1 = x_0_hat.var(dim=(3,4))
            mean2 = model_input.float().mean(dim=(3,4))
            var2 = model_input.float().var(dim=(3,4))
            color_loss = F.l1_loss(mean1, mean2, reduction="mean") + F.l1_loss(var1, var2, reduction="mean")
            print(f"loss: {loss}")
            print(f"color_loss: {color_loss}")
            beta = min(0.02, (loss.detach() / color_loss.detach()) * 0.1)
            print(f"beta: {beta}")
            loss = loss + beta * color_loss
    ...
    def train_one_step(step_, data_item_, prof_=None):
        ...
        with torch.no_grad():
            ...
            assert H_ % args.ae_stride_h == 0
            assert W_ % args.ae_stride_w == 0
            
            # mask_ratio: 0.1~0.8 random mask ratio
            mask_ratio = 0.1 + 0.7 * random.random()
            if random.random() < 0.3:
                mask_ratio_l = mask_ratio / 2.0
                mask_ratio_r = mask_ratio - mask_ratio_l
            else:
                mask_ratio_l = mask_ratio * random.random()
                mask_ratio_r = mask_ratio - mask_ratio_l
            ...
            else : # expand up and down
                mask_ratio_u = mask_ratio_l
                mask_ratio_d = mask_ratio_r
                start_h, start_w = math.floor(H_ * mask_ratio_u), 0
                end_h = math.floor(H_ * mask_ratio_d)
                mask_h_size = H_ - start_h - end_h
                mask_w_size = W_
                print(f"mask_ratio_u={mask_ratio_u}, mask_ratio_d={mask_ratio_d}")
            
            input_mask = torch.zeros_like(x)
            print(f"start_h={start_h}, mask_h_size={mask_h_size}, start_w={start_w}, mask_w_size={mask_w_size}")
            input_mask[:, :, :, start_h:start_h+mask_h_size, start_w:start_w+mask_w_size] = 1
            
            given_frame_num = 0
            if args.given_frame_max_num:
                if random.random() < 0.8:
                    given_frame_num = 0
                else:
                    given_frame_num = random.randint(1, args.given_frame_max_num)
            if given_frame_num:
                input_mask[:, :, :given_frame_num] = 1
            
            x_given = x.clone()
            x_given = x_given * input_mask
            
            x = ae.encode(x)  # B C T H W
            x_given = ae.encode(x_given)
            input_mask = ae.encode(input_mask)
            x = torch.cat((x, x_given, input_mask), dim=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--given_frame_max_num", type=int, default=0, help="The maximum number of the given frames in the beginning which are not masked.")
